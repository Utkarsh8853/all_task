...
class ProductService {
    ...
    async productEntry (name: string, description:string, price: number, quantity: number,category_id: string, seller_id:string) {
        const createProduct = await productModel.create({name: name, description:description, price: price, quantity: quantity,category_id: category_id, seller_id:seller_id});
    ...
    // async createAndSendPDF(data: string) {        
    ...
    //     const userHistory:any = await order_historyModel.findOne({buyer_id: buyer_id});
    //     if (userHistory) {               
    ...
    //         );
    //         // await order_historyModel.updateOne(
    //         //     { buyer_id: buyer_id },
    //         //     {
    //         //       $push: { history: { $each: field.cart } },
    //         //       $set: { "history.$[].address_id": address_id }
    ...
    //         { $set: { "history.$[].address_id": address_id } }
    //     );
    //     // const filter = { buyer_id: buyer_id };
    //     // const update = {
    //     //                     $push: { history: { $each: field.cart } },
    //     //                     $set: { "history.$[].address_id": address_id }
    //     //                 };
    //     // const options = { upsert: true, new: true };
    //     // await order_historyModel.findOneAndUpdate(filter, update, options);

    //     await productService.removeCart(buyer_id);
    // }

    async userHistory (buyer_id: string,address_id: string,field:any) {
        await order_historyModel.create({buyer_id:buyer_id,address_id:address_id,history:field.cart})
        await productService.removeCart(buyer_id);
    }
    ...
    async cancelOrderProduct (buyer_id: string,order_id: string,field:any) {
        await order_historyModel.updateOne({buyer_id:buyer_id,"history._id": order_id},{$set: { "history.$.delivery_status": status.cancelled}})
        await productService.updateProductQuantity(field.product_id,-field.quantity);
        
    }

    async myOrder (id:string) {
        const result = await order_historyModel.find({buyer_id:id})
        return result;
    }
    ...
    async findReviewInfo (review_id: string,buyer_id: string) {
        const result = await productModel.findOne({'review.buyer_id':buyer_id,"review._id": review_id})
        return result;
    }

    async deleteReview (user_id: string,review_id: string) {
        await productModel.updateOne({'review.buyer_id':user_id,"review._id": review_id},{$pull: {review: {_id:review_id}}})
    }
}

...
import { EmailSender } from "../utils/nodeMailer.util";
...
class AuthService {
  ...
  async userEntrybyPh_no (modelName:any,name: string, ph_no: string) {
    ...
    const token = await authService.tokenGenration(modelName,createUser._id.toString(),session_id)
    await authService.redisSessionCreation(createUser._id.toString(),session_id);
    await redis.getKey(`${ph_no}__otp`)
    return token;
  }
  ...
  async redisOtpStore (value: string,otp: string) {
    ...
    await redis.setKey(`${value}__otp`,300,JSON.stringify(otp_payload))
  }

  async otpVerification (value: string,otp: string) {
    const bcryptOtp= await redis.getKey(`${value}__otp`)as string; 
  ...
  async redisSessionCreation (user_id: string,session_id: number) {
    ...
    const a =await redis.setKey(`${user_id}_${session_id}`,24*60*60,JSON.stringify(session_payload))
  ...
  async forgetPwdToken (email: string,name:string) {
    let OTP = Math.floor(100000 + Math.random() * 9000).toString();
    try {
      const htmlTemplate= fs.readFileSync("templates/forgetPassword.template.html", "utf-8");
...
//postman test script in environment variable
// princy.sehrawat@tcs.com
...
class AuthController {
    ...
    async signinCallback(req:Request, res:Response) {
        try {
            ...
            const userEmail = ticket.getAttributes().payload?.email as string
            connect.revokeToken(tokens.access_token as string);
            console.log("/////////////",modelName);
            
            const verifyUser = await authService.verifyUserExist(modelName,userEmail)            
            if (!verifyUser){
                const jwtToken = await authService.userEntrybyGoogle(modelName,ticket.getAttributes().payload?.name as string, userEmail);
                console.log('Signup successfully',jwtToken);
                return res.status(200).send(`Here is new user.${ticket.getAttributes().payload?.name as string} whose email id is ${userEmail} and his jwt token is  "${jwtToken}" `);
            }
            else if (verifyUser){
                const jwtToken = await authService.userLoginbyThirdParty(req.baseUrl.toString(),verifyUser._id);
                console.log('Signup successfully',jwtToken);
                return res.status(200).send(`Welcome ${ticket.getAttributes().payload?.name as string} your jwt token is  "${jwtToken}"`);
            }
        } catch(err) {
            console.error(err);
        }
    ...
    async otpVerification(req:Request, res:Response) {
        try {
            ...
            if (storedOtp) {
                const verifyUser = await authService.verifyUserPh_no(modelName,ph_no);
                if (!verifyUser){
                    if(!req.body.name){
    ...
    async login(req:Request, res:Response) {
        try {
            ...
            if(pwdMatch) {
                ...
                const token = await authService.tokenGenration(req.baseUrl.toString(),verifyUser.id,sessionId);
    ...
    async forgetPassword(req:Request, res:Response) {
        try {
            ...
            const userEmail = await authService.verifyUserExist(modelName,email)
            if(!userEmail) {
                return res.status(200).json({message: "Wrong user"});
            }
            const response = await authService.forgetPwdToken(email,userEmail.name)

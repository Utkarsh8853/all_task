...
import { Cart } from "../database/models/mongoDB/cart.model";
import { Seller } from "../database/models/mongoDB/sellers.model";
import { Product } from "../database/models/mongoDB/product.model";


class ProductController {

    async viewAllCategory (req:Request, res:Response) {
        try {
            const result = await productService.viewCategory();
    ...
    async addProduct(req:Request, res:Response) {
        try {
            ...
            const isVerify:Seller = await productService.isVerifiedUser(user_id);
            if (isVerify) {
                ...
                if (verifyCategory) {
                    const newProduct = await productService.productEntry(name, description, price, quantity, category_id, user_id)
    ...
    async updateProduct(req:Request, res:Response) {
        try {
            ...
            const productDetail:Product = await productService.getProduct(product_id)
            if( productDetail){
                const productOwnership = await productService.productOwner(product_id, user_id);
                if(productOwnership){
    ...
    async removeFromCart(req:Request, res:Response) {
        try {
            ...
            const user_id = userData.id;;
            const {product_id} = req.body;
            const productDetail = await productService.cartProduct(user_id,product_id);
            if (!productDetail) {
                return res.status(400).send("No product found")
            }
    ...
    async updateQuantity(req:Request, res:Response) {
        try {
            ...
            const productDetail:Cart = await productService.cartProduct(user_id,product_id);
            if (!productDetail) {
                return res.status(400).send("No product found")
            }
            const result = await productService.findCartProductInfo(product_id,user_id);
            const checkQuantity = await productService.checkProductAvailablity(productDetail.quantity,quantity-result[0].quantity)
            if(checkQuantity){
                const cart = await productService.updateProductCartQuantity(user_id,product_id,quantity,productDetail.unit_price,result[0].quantity,result[0].total_cost)
                console.log("Product is added to cart ",cart);
                return res.status(200).json({cart});
            }
    ...
    async review(req:Request, res:Response) {
        try {
            ...
            const orderDetail = await productService.findOrderById(user_id,order_id);
            if (orderDetail.delivery_status === status.pending) {
    ...
    async deleteReview(req:Request, res:Response) {
        try {
            ...
            const reviewExist = await productService.findReviewInfo(review_id,user_id)
            if(!reviewExist){
                return res.status(400).send("No review found")
            }
            await productService.deleteReview(user_id,review_id);
            return res.status(200).send("review is added")
        } catch(err) {
            console.error(err);
            return res.status(400).send("Server problem")
        }
